...
WHERE
 ...
 EstBlf IN ('EST') AND if(SaleNo>=1 AND SaleNo<=13,53+SaleNo,SaleNo) between 14 and 63
 #(Select max(if(SaleNo>=1 AND SaleNo<=13,53+SaleNo,SaleNo)) from `data-warehousing-prod.EasyReports.SaleTransactionView` where Season = 2025 and FinYear = "2025-26")


  #AND NOT (Centre = "KOL" AND if(SaleNo>=1 AND SaleNo<=13,53+SaleNo,SaleNo) = 59 AND Season <= 2025)
...
excel_path = "DO_TR_EST.xlsx"
filtered_df.to_excel(excel_path, index=True,sheet_name="DO EST")
