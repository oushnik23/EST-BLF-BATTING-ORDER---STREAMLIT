...
Case when CAST(substring(FinYear,1,4) as INT64) = Season then Season else 0 end as FYear,
    ...
    CASE WHEN SaleNo BETWEEN 1 AND 13 THEN 53 + SaleNo ELSE SaleNo END AS SaleAlies,
...
WHERE
 Season IN (2025,2024) AND Area IN ("AS") AND Centre IN ("KOL","GUW") AND Category IN ('CTC') AND EstBlf IN ('EST')
AND if(SaleNo>=1 AND SaleNo<=13,53+SaleNo,SaleNo) between 14 and 63
...
# Apply ranking for each year based on Sold_Qty >= 100000
for year in sold_qty.columns:
    valid_rows = sold_qty[year] >= 100000
...
heading_cell.value = "Report Name: Top Batting Order"
...
value = df['SaleAlies'].max() - 53 if df['SaleAlies'].max() > 53 else df['SaleAlies'].max()
...
for i, (label, val) in enumerate(info_text, start=start_row):
    # Label in A
    label_cell = ws.cell(row=i, column=1)
    label_cell.value = label
    label_cell.font = Font(size=12)
    label_cell.alignment = Alignment(horizontal="left", vertical="center")

    # Value in B (bold for Area & Est / Blf only)
    val_cell = ws.cell(row=i, column=2)
    val_cell.value = val
    val_cell.font = Font(size=12, bold=(label in ["Area:", "Est / Blf:"]))
    val_cell.alignment = Alignment(horizontal="left", vertical="center")

    # Merge A–I per row for clean layout, but keep text visible from A+B
    ws.merge_cells(start_row=i, start_column=3, end_row=i, end_column=9)

# Adjust row heights
for row in range(start_row, start_row + len(info_text)):
