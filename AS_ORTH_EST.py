...
WHERE
 Season IN (2025,2024) AND Area IN ("AS") AND Centre IN ("KOL","GUW") AND Category IN ('ORTHODOX') AND 
...
# Apply ranking for each year based on Sold_Qty >= 75000
for year in sold_qty.columns:
    valid_rows = sold_qty[year] >= 75000
...
excel_path = "AS_ORTH_EST.xlsx"
filtered_df.to_excel(excel_path, index=True,sheet_name="AS ORTH EST")
